...
public class TribeDinner {
    ...
    // da se static)
    static Semaphore zemanjeHrana;
    static Semaphore proverkaHrana;
    ...
    public static void init(int numBarbers) {
        zemanjeHrana = new Semaphore(3);
        proverkaHrana = new Semaphore(1);
    }
    ...
    static class TribeMember extends TemplateThread {
        ...
        public void execute() throws InterruptedException {
            proverkaHrana.acquire();
            boolean prazno = state.isPotEmpty();
            proverkaHrana.release();

            if (!prazno){
                zemanjeHrana.acquire();
                state.fillPlate();
                zemanjeHrana.release();
            }
            if (!prazno)
                state.eat();
            else
